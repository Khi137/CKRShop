...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
  ...
  numberStocks: number;
  numberTrademark: number;
  constructor(
    private changeDetectorRef: ChangeDetectorRef, private api: ApiService
  ) {
    this.api.getCountProducts().subscribe({
      next: (res) => {
        this.numberProducts = res;
      }
    })
    this.api.getCountCategories().subscribe({
      next: (res) => {
        this.numberCategories = res;
      }
    })
    this.api.getCountTrademarks().subscribe({
      next: (res) => {
        this.numberTrademark = res;
      }
    })
    this.api.getSumStocks().subscribe({
      next: (res) => {
        this.numberStocks = res;
      }
    })
  }

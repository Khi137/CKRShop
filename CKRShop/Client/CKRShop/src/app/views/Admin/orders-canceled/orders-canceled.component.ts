...
import { OrderCanceledDialogComponent } from './order-canceled-dialog/order-canceled-dialog.component';
@Component({
...
export class OrdersCanceledComponent implements OnInit {
  ...
  openDialog() {
    this.dialog.open(OrderCanceledDialogComponent, {
      width: "30%",
      position: {
        top: '8%',
        left: '40%'
  ...
  getAllOrders() {
    //this.getAllUsers();
    this.api.getAllOrdersCanceled().subscribe({
      next: (res) => {
